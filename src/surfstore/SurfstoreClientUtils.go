...
//	log.Print("Deleting File")
	...
	for _,element := range temp_FileMetaMap {
		...
		if _, err := os.Stat(client.BaseDir+"/"+element.Filename); err == nil {
			...
			temp_FileMetaData.BlockHashList = []string{"0"}
			temp_FileMetaData.Version += 1
			log.Print(temp_FileMetaData.Version)
			err := client.UpdateFile(&temp_FileMetaData,&delete_version)
			log.Print(delete_version)
			if err != nil{
				UpdateLocal(client, element)
			} else {
				version := strconv.Itoa(delete_version)
